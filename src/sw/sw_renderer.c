...
static int32_t g_DynamicSurfaceSpriteSlot = -1;
                                  ...
                                  float angleDeg, uint32_t color, float alpha)
{
        ...
        if (tpagIndex == 9999) 
        {
                ...
                // For dynamic surfaces, source dimensions are the full texture size
                sx = 0;
                sy = 0;
                sw = texture->width;
                sh = texture->height;

                // Emulate typical layout positioning based on passed origins
                dx = (float)(0 - originX);
                dy = (float)(0 - originY);
                dw = (int)(xscale * sw);
                dh = (int)(yscale * sh);
                ...
                return; // Dynamic surface draw finished safely!
        }
        ...
        sy = tpag->sourceY;
        sw = tpag->sourceWidth;
        sh = tpag->sourceHeight;

        dx = (float)(tpag->targetX - originX);
        dy = (float)(tpag->targetY - originY);
        dw = (int)(xscale * tpag->targetWidth);
        dh = (int)(yscale * tpag->targetHeight);
        dx *= xscale;
        dy *= yscale;
        dx += x;
        dy += y;
                                                   ...
                                                   int32_t xorig, int32_t yorig)
{
        ...
        sandboxTex = swrCreateTexture(NULL, w, h);
        if (!sandboxTex || !sandboxTex->buffer) {
                return 0;
        }
...
Renderer* SWRenderer_create(void)
{
        ...
        // FIX: If your base Renderer class contains the vtable instance storage directly,
        // assign the pointers directly to it. If it expects a pointer to an isolated vtable,
        // make sure the target struct is declared as a persistent 'static' global block so it never drops off the stack!
        static RendererVtable safePersistentVtable;
        swr->base.vtable = &safePersistentVtable;

        safePersistentVtable.init                     = SWRenderer_init;
        safePersistentVtable.destroy                  = SWRenderer_destroy;
        safePersistentVtable.beginFrame               = SWRenderer_beginFrame;
        safePersistentVtable.endFrameInit             = SWRenderer_endFrameInit;
        safePersistentVtable.endFrameEnd              = SWRenderer_endFrameEnd;
        safePersistentVtable.beginView                = SWRenderer_beginView;
        safePersistentVtable.endView                  = SWRenderer_endView;
        safePersistentVtable.beginGUI                 = SWRenderer_beginGUI;
        safePersistentVtable.setGuiProjection         = SWRenderer_setGuiProjection;
        safePersistentVtable.endGUI                   = SWRenderer_endGUI;
        safePersistentVtable.drawSprite               = SWRenderer_drawSprite;
        safePersistentVtable.drawSpritePart           = SWRenderer_drawSpritePart;
        safePersistentVtable.drawSpritePos            = SWRenderer_drawSpritePos;
        safePersistentVtable.drawRectangle            = SWRenderer_drawRectangle;
        safePersistentVtable.drawRectangleColor       = SWRenderer_drawRectangleColor;
        safePersistentVtable.drawLine                 = SWRenderer_drawLine;
        safePersistentVtable.drawTriangle             = SWRenderer_drawTriangle;
        safePersistentVtable.drawLineColor            = SWRenderer_drawLineColor;
        safePersistentVtable.drawText                 = SWRenderer_drawText;
        safePersistentVtable.drawTextColor            = SWRenderer_drawTextColor;
        safePersistentVtable.flush                    = SWRenderer_flush;
        safePersistentVtable.clearScreen              = SWRenderer_clearScreen;
        safePersistentVtable.createSpriteFromSurface  = SWRenderer_createSpriteFromSurface;
        safePersistentVtable.deleteSprite             = SWRenderer_deleteSprite;
        safePersistentVtable.gpuSetBlendMode          = SWRenderer_gpuSetBlendMode;
        safePersistentVtable.gpuSetBlendModeExt       = SWRenderer_gpuSetBlendModeExt;
        safePersistentVtable.gpuSetBlendEnable         = SWRenderer_gpuSetBlendEnable;
        safePersistentVtable.gpuSetAlphaTestEnable     = SWRenderer_gpuSetAlphaTestEnable;
        safePersistentVtable.gpuSetAlphaTestRef        = SWRenderer_gpuSetAlphaTestRef;
        safePersistentVtable.gpuSetColorWriteEnable    = SWRenderer_gpuSetColorWriteEnable;
        safePersistentVtable.gpuGetColorWriteEnable    = SWRenderer_gpuGetColorWriteEnable;
        safePersistentVtable.gpuGetBlendEnable         = SWRenderer_gpuGetBlendEnable;
        safePersistentVtable.gpuSetFog                 = SWRenderer_gpuSetFog;
        safePersistentVtable.drawTile                 = NULL;
        safePersistentVtable.drawTiled                = SWRenderer_drawTiled;
        safePersistentVtable.createSurface            = SWRenderer_createSurface;
        safePersistentVtable.surfaceExists            = SWRenderer_surfaceExists;
        safePersistentVtable.setRenderTarget          = SWRenderer_setRenderTarget;
        safePersistentVtable.ensureApplicationSurface = SWRenderer_ensureApplicationSurface;
        safePersistentVtable.getSurfaceWidth          = SWRenderer_getSurfaceWidth;
        safePersistentVtable.getSurfaceHeight         = SWRenderer_getSurfaceHeight;
        safePersistentVtable.drawSurface              = SWRenderer_drawSurface;
        safePersistentVtable.surfaceResize            = SWRenderer_surfaceResize;
        safePersistentVtable.surfaceFree              = SWRenderer_surfaceFree;
        safePersistentVtable.surfaceCopy              = SWRenderer_surfaceCopy;
        safePersistentVtable.surfaceGetPixels         = SWRenderer_surfaceGetPixels;
        safePersistentVtable.drawTiledPart            = SWRenderer_drawTiledPart;
        safePersistentVtable.spriteGetTexture         = SWRenderer_spriteGetTexture;
        safePersistentVtable.surfaceGetTexture        = SWRenderer_surfaceGetTexture;
        safePersistentVtable.textureGetTexelWidth     = SWRenderer_textureGetTexelWidth;
        safePersistentVtable.textureGetTexelHeight    = SWRenderer_textureGetTexelHeight;
        safePersistentVtable.textureGetUVs            = SWRenderer_textureGetUVs;
        safePersistentVtable.textureSetStage          = SWRenderer_textureSetStage;
        safePersistentVtable.gpuSetShader             = SWRenderer_gpuSetShader;
        safePersistentVtable.gpuResetShader           = SWRenderer_gpuResetShader;
        safePersistentVtable.shaderGetUniform         = SWRenderer_shaderGetUniform;
        safePersistentVtable.shaderGetSamplerIndex    = SWRenderer_shaderGetSamplerIndex;
        safePersistentVtable.shaderSetUniformF        = SWRenderer_shaderSetUniformF;
        safePersistentVtable.shaderSetUniformI        = SWRenderer_shaderSetUniformI;
        safePersistentVtable.shaderIsCompiled         = SWRenderer_shaderIsCompiled;
        safePersistentVtable.shadersSupported         = SWRenderer_shadersSupported;
