...
#define TEXTURE_LRU_LENGTH 128

#ifndef UNIMP2
#define UNIMP2() do { fprintf(stderr, "NYI %s\n", __func__); } while (0)
#endif
...
{
    SWTexture* tex = (SWTexture*) safeCalloc(1, sizeof(SWTexture));
    if (!tex)
        return NULL;

    tex->masterPageId = pageId;
    tex->width = width;
    tex->height = height;

    // Allocate array pointers for 64 tile slices (8x8 grid of 256x256 slices)
    //tex->slices = (uint16_t**) safeCalloc(64, sizeof(uint16_t*));
    //tex->sliceCount = 64;
...
static uint16_t* swrStreamSliceFromDisk(SWTexture* parentTex, int sliceIndex, int sliceSize)
{
    ...
    if (!g_TextureArchive) {
        ...
        if (!g_TextureArchive) {
            fprintf(stderr, "[SWR-ERROR] FAILED TO OPEN /roms/butterscotch/texture_archive.pak!\n");
            fflush(stderr);
        } else {
            fprintf(stderr, "[SWR] Successfully opened /roms/butterscotch/texture_archive.pak!\n");
            fflush(stderr);
        }
    }
...
static inline uintpixel_t swrGetVirtualTexel(SWTexture* texture, int x, int y)
{
    ...
    // Handle dynamic surfaces/framebuffers
    if (texture->buffer) {
        if (x < 0 || x >= texture->width || y < 0 || y >= texture->height) return 0;
        return texture->buffer[y * texture->width + x];
    }

    // Handle streamed preprocessed textures
    if (x < 0 || x >= texture->width || y < 0 || y >= texture->height) return 0;

    int sliceX = x / 256;
    int sliceY = y / 256;
    int sliceIdx = (sliceY * 8) + sliceX;

    if (!texture->slices[sliceIdx]) {
        // Calls the existing 3-argument function at line 306:
        texture->slices[sliceIdx] = swrStreamSliceFromDisk(texture, sliceIdx, 256);
    }

    int localX = x % 256;
    int localY = y % 256;
    return texture->slices[sliceIdx][(localY * 256) + localX];
}

// Lazily decodes and uploads a TXTR page on first access using 16-bit streaming.
...
{
    // Fast path: texture virtual canvas is already registered
    if (swr->textures[pageId])
        return true;

    // Bounds check to ensure pageId is safe
    if (pageId >= swr->totalTextureCount) {
        fprintf(stderr, "swr: Invalid TXTR pageId %u requested\n", pageId);
        return false;
    }

    // Default canvas dimension for GameMaker texture pages
    int nativeWidth = 2048;
    int nativeHeight = 2048;

    // Bypasses ImageDecoder_decodeToRgba completely!
    // Instantiates a zero-RAM lightweight container; slices stream on demand via swrStreamSliceFromDisk.
    SWTexture* tex = swrCreateTexturePreprocessed(pageId, nativeWidth, nativeHeight);
    if (!tex) {
        fprintf(stderr, "swr: Failed to create virtual texture container for page %u\n", pageId);
        return false;
    }

    swr->textures[pageId] = tex;

    // Track usage in the LRU cache
    while (!swrAddTextureIndexToLRU(swr, pageId)) {
        int tail = swrTailTextureIndexLRU(swr, true);
        if (tail == -1) {
            break; // LRU empty or full without eviction target
        }
...
static void SWRenderer_init(Renderer* renderer, DataWin* dataWin)
{
    ...
    // Viewport and scaling defaults
    swr->viewActive = false;
    swr->viewX = 0; swr->viewY = 0; swr->viewW = 0; swr->viewH = 0;
    swr->portX = 0; swr->portY = 0; swr->portW = 0; swr->portH = 0;
    swr->gameW = 0; swr->gameH = 0; swr->maxX = 0;  swr->maxY = 0;
    swr->scaleX = 1.0f; swr->scaleY = 1.0f;
    swr->offsetX = 0;   swr->offsetY = 0;
    swr->defaultScaleX = 1.0f; swr->defaultScaleY = 1.0f;

    // Expand datawin's tpag items list to include surface count
...
static void SWRenderer_destroy(Renderer* renderer)
{
    ...
    // Free texture containers and slice allocations
    if (swr->textures) {
        for (size_t i = 0; i < swr->totalTextureCount; i++) {
            if (swr->textures[i]) {
                // Free slice buffers for streamed textures
                for (int s = 0; s < 64; s++) {
                    if (swr->textures[i]->slices[s]) {
                        free(swr->textures[i]->slices[s]);
                        swr->textures[i]->slices[s] = NULL;
                    }
                }
                // Free dynamic buffer if present (surfaces)
                if (swr->textures[i]->buffer) {
                    free(swr->textures[i]->buffer);
                }
                free(swr->textures[i]);
            }
        }
    ...
    // Free surface pointer tracking array
    if (swr->surfaces) {
        free(swr->surfaces);
        swr->surfaces = NULL;
    }

    // Free LRU cache ring buffer
    if (swr->textureIndexLRU) {
        free(swr->textureIndexLRU);
        swr->textureIndexLRU = NULL;
    }

    // Close global stream archive if open
    if (g_TextureArchive) {
        fclose(g_TextureArchive);
        g_TextureArchive = NULL;
    }
...
)
{
    ...
    bool isDynamicBuffer = (texture->buffer != NULL);
    ...
    if (sw == dw)
    {
        ...
        for (int y = 0, ys = iys; y < dh; y++, ys += oys, ys2 += oys2)
        {
            ...
            int srcY = sy + ((dh == sh) ? ys : (int)(ys2 >> fp_prec));

            if (isDynamicBuffer) {
                const uintpixel_t* srcline = &texture->buffer[srcY * texture->width + sx];
                ...
                {
                    int srcX = sx + xs;
                    uintpixel_t pixel = swrGetVirtualTexel(texture, srcX, srcY);
                    if (opaque(pixel))
                        alphaBlend(&dstline[x], tint(tintColor, pixel), alpha);
                }
            }
                                   ...
                                   uint32_t color, float alpha)
{
    ...
    if (surfaceID == APPLICATION_SURFACE_ID) {
        // Fix: When drawingToSurface is false, the application surface data
        // resides in mainFb (the offscreen buffer set during rendering).
        localSurface.buffer = swr->drawingToSurface ? swr->fb : swr->mainFb;
        localSurface.width = swr->drawingToSurface ? swr->width : swr->mainWidth;
        localSurface.height = swr->drawingToSurface ? swr->height : swr->mainHeight;
        surface = &localSurface;

        fprintf(stderr, "[SWR] Drawing APP_SURFACE: drawingToSurface=%d, buf=%p, dims=%dx%d\n",
                swr->drawingToSurface, (void*)surface->buffer, surface->width, surface->height);
    } else {
        if (surfaceID < 0 || (size_t) surfaceID >= swr->surfaceCount || swr->surfaces[surfaceID] == NULL) {
        ...
        fprintf(stderr, "[SWR] Drawing surface ID %d: buf=%p, dims=%dx%d\n",
                surfaceID, (void*)surface->buffer, surface->width, surface->height);
    }
